...
#include "TFile.h"
#include "TGraph.h"
void SigmaGraphPlotter()
{ 
  TCanvas * canvas_sigmas = new TCanvas("canvas_sigmas","canvas_sigmas",1200,1200);
  canvas_sigmas->Divide(2,2);

  TFile * file_2sigma = TFile::Open("output_run61_500k_2sigma.root");
  TFile * file_3sigma = TFile::Open("output_run61_500k_3sigma.root");
  TFile * file_4sigma = TFile::Open("output_run61_500k_4sigma.root");
  TFile * file_5sigma = TFile::Open("output_run61_500k_5sigma.root");
  ...
  TBox * box2 = new TBox(-2.02-13/2,-1.01-13/2,-2.02+13/2,-1.01+13/2);
  box2->SetFillStyle(0);
  ...
  TBox * box3 = new TBox(-2.02-13/2,-1.01-13/2,-2.02+13/2,-1.01+13/2);
  box3->SetFillStyle(0);
  ...
  TBox * box4 = new TBox(-2.02-13/2,-1.01-13/2,-2.02+13/2,-1.01+13/2);
  box4->SetFillStyle(0);
  ...
  TBox * box5 = new TBox(-2.02-13/2,-1.01-13/2,-2.02+13/2,-1.01+13/2);
  box5->SetFillStyle(0);
