...
bool is_good_strip(UInt_t det, UInt_t strip)
{
    switch(det)
    {
        ...
        case 16:
            if(
                    strip==8 || strip==16 || strip==114 || (strip>139 && strip<146) || strip==229 || strip==462 || strip==463 || strip==464 || strip==229 || strip==230 ||
...
void analyse(int firstEvent, int max_events, TChain * ch)
{
    ...
    for(int i=0; i<NDETS; i++)
    {
        ...
        h1_cluster_e[i]    = new TH1D(Form("h1_cluster_e_FOOT%d",foot_id[i]),Form("h1_cluster_e_FOOT%d",foot_id[i]),200,0,100);
        h1_cluster_size[i] = new TH1I(Form("h1_cluster_size_FOOT%d",foot_id[i]),Form("h1_cluster_size_FOOT%d",foot_id[i]),10,0,10);
        h2_cluster_e_vs_cog[i] = new TH2D(Form("h2_cluster_e_vs_cog_FOOT%d",foot_id[i]),Form("h2_cluster_e_vs_cog_FOOT%d",foot_id[i]),640,1,641,100,0,100);
        h1_cluster_mul[i] = new TH1I(Form("h1_cluster_mul_FOOT%d",foot_id[i]),Form("h1_cluster_mul_FOOT%d",foot_id[i]),40,0,20);
    }
    ...
    h1_beam_X = new TH1D(Form("h1_beam_X"),Form("h1_beam_X"),100,-50,50);
    h1_beam_Y = new TH1D(Form("h1_beam_Y"),Form("h1_beam_Y"),100,-50,50);
    ...
    for(int ev=firstEvent; ev<firstEvent+Nevents; ev++)
    {
        ...
        if(data[0].size()<mul_max && data[1].size()<mul_max)
        {
            for(auto & c0: data[0])
            {
                for(auto & c1: data[1])
	...
	h1_beam_X->Fill(X);
        ...
        for(auto & c1: data[1]){   
            xp = get_cog(c1);
            X = (-1)*(xp*FOOT_LENGTH/640.- FOOT_LENGTH/2.);
            if(X>Xmin && X<Xmax && get_esum(c1)>10){
                is16 = true;
                break;
            }
        }
        if(is16){
            ...
            for(auto & c0: data[0]){
                Y = (get_cog(c0)*FOOT_LENGTH/640.- FOOT_LENGTH/2.);
                if(Y>Ymin && Y<Ymax){
                    h1_cluster_e[0]->Fill( get_esum(c0) );
                    is15 = true;
                    break;
                }
            }
        ...
        F15_mul = 0;
        F16_mul = 0; 
        h1_cluster_mul[0]->Fill(data[0].size());
        h1_cluster_mul[1]->Fill(data[1].size());
        if(data[0].size()<mul_max && data[1].size()<mul_max)
        {
            for(int f=0; f<NDETS; f++)
            {
    ...
    for(int f=0; f<NDETS; f++)
    {
        ...
        //gPad->SetLogy();
        h1_cluster_e[f]->SetFillColor(kGreen);
        h1_cluster_e[f]->Draw();

        canvas_cluster_size->cd(f+1);
        h1_cluster_size[f]->SetFillColor(kRed);
    ...
    canvas_det_mul->cd();
    h1_det_mul->SetFillColor(kRed);
    h1_det_mul->Scale(100./h1_det_mul->Integral());
    h1_det_mul->Draw("HIST");

    TCanvas* canvas_cluster_mul   = new TCanvas("cluster_mul","cluster_mul",1800,1200);
    canvas_cluster_mul->Divide(2,1);
    for(int i=0; i<NDETS; i++)
    {
        canvas_cluster_mul->cd(i+1);
        h1_cluster_mul[i]->SetFillColor(kRed);
        h1_cluster_mul[i]->Draw();
    }

    cout << "\n\n Efficiency of F16: " << (double)N16_if_15/(double)N15*100 << "%\n\n";
    cout << "\n\n Efficiency of F15: " << (double)N15_if_16/(double)N16*100 << "%\n\n";
